...
func generate_maze():
	...
	while frontier.size() > 0:
		...
		#print_neighbors(curr_cell,neighbors)
		...
		mark(curr_cell)
func mark(c : Cell):
	print("MARKING: ", c.position)
	c.in_maze = true
	# Add its surrounding cells to the frontier (idk if it has to be checked if they are in the maze already)
